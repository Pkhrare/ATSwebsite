...
export default function TaskCard({ task, onClose, onTaskUpdate, assigneeOptions, isClientView = false, isEditable = false }) {
                    ...
                    <div className="p-6 border-b border-gray-600 bg-gray-800">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3 min-w-0">
                                <svg className="w-6 h-6 text-yellow-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    ...
    return (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-2 md:p-4" onClick={handleOverlayClick}>
            <div ref={modalContentRef} className="bg-gray-50 text-gray-800 border border-gray-200 rounded-lg shadow-xl w-full max-w-5xl flex flex-col relative" style={{ maxHeight: '95vh' }} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-grow flex overflow-hidden">
                    <form onSubmit={handleSubmit} className="flex-grow p-3 md:p-6 overflow-y-auto space-y-4 md:space-y-6 custom-scrollbar">
                        <div className="space-y-4 md:space-y-6">
                            ...
                            {/* Mobile-only Add Task Fields Section */}
                            {!isClientView && (
                                <div className="lg:hidden p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            onClick={() => setAttachChecklistsFormOpen(true)}
                                            className="flex items-center gap-2 p-2 text-xs bg-white border border-yellow-300 rounded-md hover:bg-yellow-100 transition-colors"
                                        >
                                        ...
                                        <button
                                            onClick={() => setAttachFilesFormOpen(true)}
                                            className="flex items-center gap-2 p-2 text-xs bg-white border border-yellow-300 rounded-md hover:bg-yellow-100 transition-colors"
                                        >
                                        ...
                                        <button
                                            onClick={() => setAttachTaskformsFormOpen(true)}
                                            className="flex items-center gap-2 p-2 text-xs bg-white border border-yellow-300 rounded-md hover:bg-yellow-100 transition-colors"
                                        >
                                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                        ...
                                        <button
                                            onClick={() => setAddApprovalFormOpen(true)}
                                            className="flex items-center gap-2 p-2 text-xs bg-white border border-yellow-300 rounded-md hover:bg-yellow-100 transition-colors"
                                        >
                                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-4">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="p-2 md:p-3 bg-white rounded-md border w-1/3">
                                        <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase flex items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-1.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                            Time Remaining
                                        </label>
                                        <div className={`text-sm md:text-lg font-bold ${calculateDaysRemaining().includes('overdue') ? 'text-red-600' : calculateDaysRemaining() === 'Due today' ? 'text-orange-600' : 'text-black'}`}>{calculateDaysRemaining()}</div>
                                    </div>
                                </div>
                                <div>
                                ...
                                <div className="mt-6 pt-6 border-t">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                                        {/* Mobile: Stack vertically, Desktop: Delete on left, others on right */}
                                        <div className="flex flex-col sm:flex-row gap-3 order-2 sm:order-1">
                                            {!isClientView && (
                                                <button type="button" onClick={handleDeleteTask} className="w-full sm:w-auto px-4 py-3 bg-red-600 text-white font-medium text-sm rounded-lg hover:bg-red-700">
                                                    Delete Task
                                                </button>
                                            )}
                                        </div>
                                        <div className="flex flex-col sm:flex-row gap-3 order-1 sm:order-2">
                                            <button type="button" onClick={onClose} className="w-full sm:w-auto px-4 py-3 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 font-medium" disabled={isLoading}>Cancel</button>
                                            {canEdit && (
                                                <button type="submit" className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300 font-medium" disabled={isLoading}>
                    ...
                    {!isClientView && !isAddTaskFieldsCollapsed && (
                        <div className={`hidden lg:block w-80 border-l p-4 ${colorClasses.button.secondary} border-2 border-yellow-500`}>
                            {/* Collapsible Header */}
                            <div 
                ...
                {/* Floating Add Task Fields Button - Only shows on desktop when collapsed */}
                {!isClientView && isAddTaskFieldsCollapsed && (
                    <button
                        onClick={() => setIsAddTaskFieldsCollapsed(false)}
                        className="hidden lg:flex absolute top-20 right-4 bg-yellow-400 hover:bg-yellow-500 text-black text-sm font-medium px-3 py-2 rounded-md shadow-md transition-all duration-200 hover:shadow-lg z-10 flex-col items-center gap-1"
